#!/usr/bin/env bash
# Runs each script in scripts/sf/changes/ exactly once.
# Applied scripts are tracked in /var/lib/sovereignflow/applied-changes/sf/.
# If an applied script is modified, the run fails — create a new change script instead.
set -euo pipefail

CHANGES_DIR="$(cd "$(dirname "$0")/changes" && pwd)"
APPLIED_DIR="/var/lib/sovereignflow/applied-changes/sf"
